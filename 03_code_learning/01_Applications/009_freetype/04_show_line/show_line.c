//? 使用freetype显示一行文字
//? arm-buildroot-linux-gnueabihf-gcc -lfreetype -o show_line show_line.c
#include <fcntl.h>
#include <ft2build.h>
#include <linux/fb.h>
#include <math.h>
...
int compute_string_bbox(FT_Face face, wchar_t *wstr, FT_BBox *abbox) {
  ...
  for (i = 0; i < wcslen(wstr); i++) {
    ...
    /* 取出glyph */
    error = FT_Get_Glyph(
        face->glyph, &glyph);  //- 从face的插槽glyph中获取字符信息保存至glyph中
    if (error) {
...
int display_string(FT_Face face, wchar_t *wstr, int lcd_x, int lcd_y) {
  ...
  FT_GlyphSlot slot = face->glyph;  //- 对应字符的处理结果:含glyph和位图
...
int main(int argc, char **argv) {
  ...
  if (argc == 5) font_size = strtoul(argv[4], NULL, 0);
