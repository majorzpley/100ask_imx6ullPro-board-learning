...
static ssize_t gpio_key_read(struct file *file, char __user *buf, size_t size,
  ...
  // 若g_condition不为true就会进入休眠
  wait_event_interruptible(gpio_key_wait, g_condition);
  err = copy_to_user(buf, &g_condition, sizeof(g_condition));
  g_condition = 0;
