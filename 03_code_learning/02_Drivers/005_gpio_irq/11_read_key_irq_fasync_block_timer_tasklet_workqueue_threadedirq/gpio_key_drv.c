...
struct gpio_key {
  ...
  struct tasklet_struct tasklet; /*中断下半部tasklet*/
...
static irqreturn_t gpio_key_isr(int irq, void *dev_id) {
  ...
  /*使用threaded irq中断处理的时候，需要返回另一个参数*/
  // return IRQ_HANDLED;
  return IRQ_WAKE_THREAD;
}

/*内核底半部中断处理函数*/
static irqreturn_t gpio_key_threaded_func(int irq, void *data) {
  printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
  ...
  // 打印当前线程的名字和pid
  printk("%s: the process is \"%s\"(pid %d)\n", __FUNCTION__, current->comm,
         current->pid);
...
static int gpio_key_probe(struct platform_device *pdev) {
  ...
  for (i = 0; i < count; i++) {
    ...
    // 初始化work
    INIT_WORK(&gpio_key_100ask[i].work, key_work_func);
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    // 使用线程化的顶半部底半部中断处理函数
    err = request_threaded_irq(gpio_key_100ask[i].irq, gpio_key_isr,
                               gpio_key_threaded_func,
                               IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,
                               "100ask_gpio_keys", &gpio_key_100ask[i]);
  }
